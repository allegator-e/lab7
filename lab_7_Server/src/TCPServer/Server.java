                        ...
                        try {
                            ...
                            if (key.isValid() && key.isAcceptable()) {
                                ...
                                client.register(selector, SelectionKey.OP_READ);
                                    ...
                                    service_in.submit(new TCPServerReceiver(key, connection, serverCollection, history, forkJoinPool));

                            } else if (key.isValid() && key.isWritable()) {
                                if (!(key.attachment() == null)) {
                                    ArrayList<Object> attach = (ArrayList<Object>) key.attachment();
                                    service_out.execute(new TCPServerSender((Boolean)attach.get(0),(String) attach.get(1), key));
