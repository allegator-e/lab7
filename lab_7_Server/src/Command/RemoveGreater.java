    ...
    public String execute(Object args) {
        ...
        synchronized (getManager().getHouses()) {
            if (getManager().getHouses().size() != 0) {
                if (getManager().getHouses().keySet().stream().anyMatch(key -> getManager().getHouses().get(key).compareTo(flat) > 0)) {
                    try {
                        ...
                        getManager().getHouses().keySet().stream()
                                .filter(key -> getManager().getHouses().get(key).compareTo(flat) > 0)
                                .peek(key -> {
                                    try {
